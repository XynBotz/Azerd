...
# // USERNAME IZIN IPP
rm -f /usr/bin/user
username=$(curl -sS https://raw.githubusercontent.com/LunaticTunnel/Azerd/ABSTRAK/REGIST | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
rm -f /usr/bin/e
valid=$(curl -sS https://raw.githubusercontent.com/LunaticTunnel/Azerd/ABSTRAK/REGIST | grep $MYIP | awk '{print $3}')
...
REVISI="https://raw.githubusercontent.com/LunaticTunnel/Azerd/ABSTRAK/"
...
echo -e " ${CYAN}│$NC • $NC${WHITE} City ${NC}          ${CYAN}=$NC $CITY${NC}"
echo -e " ${CYAN}│$NC • $NC${WHITE} Cpu Usage ${NC}     ${CYAN}=$NC $cpu_usage${NC}"
echo -e " ${CYAN}│$NC • $NC${CYAN} Domain ${NC}        ${CYAN}=$NC $domain${NC}"
